		...
		for x in len(level[y]):
			match level[y][x]:
				e:
					pass
				b:
					if level_difficulty % step_size == 0:
						tile.set_cell(x, y, level_difficulty/step_size)
					elif level_difficulty % step_size == 1:
						if randi() % step_size <= 1:
							tile.set_cell(x, y, floor(level_difficulty/step_size))
						else:
							tile.set_cell(x, y, floor(level_difficulty/step_size)+1)
					else:
